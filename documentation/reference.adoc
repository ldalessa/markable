...
`m1 == m2` <=> `!m1.has_value() ? !m2.has_value() : m2.has_value() && m1.value() == m2.value()`.

`m1 < m2` <=> `!m2.has_value() ? false : (!m1.has_value() ? true : m1.value() < m2.value())`.
...
`m1 == m2` <=> `m1.representation_value() == !m2.representation_value()`.
