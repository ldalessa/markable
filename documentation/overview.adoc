...
WARNING: However, there is a certain danger involved when using dual storage.
         Types `T` and `DualT` passed to `markable_dual_storage_type` need to
         preserve certain relation: they have to be layout-compatible. There is
         no way to enforce it statically in C++ prior to C++20, so you have to
         make sure this is the case. Otherwise you are risking UB.
